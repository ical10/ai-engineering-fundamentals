...
function parsedExpectedCounts(expected: string[]): Record<string, number> {
...
// Structure scorer: checks how well the produced elements match the test case's
// expected characteristics. Looks for "N type" patterns in the expected strings
// (e.g. "3 rectangle elements", "2 arrow elements") and score by how close the
// actual element counts are.
//
// Code based scorer, no LLM involved. Returns a 0-1 fractional score.
...
> = ({ output, expected }) => {
  ...
  if (!expected) {
    return {
      ...
      score: 0.5,
      metadata: {
        reason: "no expected provided",
      },
    };
  }

  const expectedCounts = parsedExpectedCounts(expected.expectedCharacteristics);
  ...
  if (Object.keys(expectedCounts).length === 0) {
    return {
      ...
      score: 0.5,
      metadata: {
        reason: "no countable expectations",
        actualCounts,
      },
    };
  }
