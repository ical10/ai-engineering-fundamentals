...
export class DesignAgent extends AIChatAgent<ENV> {
  async onChatMessage() {
    ...
    const result = streamText({
      ...
      tools,
      stopWhen: stepCountIs(5), // NOTE: handling loop internally; check out the Intro to Agent course, we handle the loop from scratch there
      providerOptions: { openai: { stringJsonSchema: false } }, // NOTE: opting out of structured outputs intentionally because we want to build robust AI systems instead; so no matter which models we switched into later on, it will be resilient
    });
